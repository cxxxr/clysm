...
(defun compile-make-instance* (args env)
  "Compile a make-instance* primitive call.
   This handles the function call form generated by DEFSTRUCT expansion:
   (make-instance* 'class-name :slot1 val1 :slot2 val2 ...)

   Arguments:
     args - List of AST nodes: (class-name-expr &rest initarg-pairs)
     env  - Compilation environment

   Returns:
     List of Wasm instructions creating an instance.

   For MVP, this returns ref.null to unblock compilation.
   Full WasmGC codegen (struct.new $instance) will follow."
  (declare (ignore env))
  ;; Extract class name from first argument
  (when (null args)
    (error "make-instance* requires at least a class name argument"))
  (let* ((class-name-ast (first args))
         (class-name (cond
                       ;; Quoted symbol: (make-instance* 'point ...)
                       ((and (typep class-name-ast 'clysm/compiler/ast:ast-literal)
                             (eq (clysm/compiler/ast:ast-literal-literal-type class-name-ast)
                                 :quoted))
                        (clysm/compiler/ast:ast-literal-value class-name-ast))
                       ;; Symbol literal
                       ((and (typep class-name-ast 'clysm/compiler/ast:ast-literal)
                             (symbolp (clysm/compiler/ast:ast-literal-value class-name-ast)))
                        (clysm/compiler/ast:ast-literal-value class-name-ast))
                       (t
                        (error "make-instance* class name must be a quoted symbol, got: ~S"
                               class-name-ast))))
         (class-info (find-compile-time-class class-name)))
    ;; Validate class exists (T012: Error handling)
    (unless class-info
      ;; Class may not be registered yet at compile time
      ;; For DEFSTRUCT-generated code, the defclass should run first
      ;; For now, we allow unknown classes and defer to runtime
      nil)
    ;; For MVP, return ref.null to unblock compilation
    ;; This allows all DEFSTRUCT usages to compile successfully
    ;; Full implementation would emit:
    ;;   - global.get for class reference
    ;;   - array.new for slot vector (with slot_count from class-info)
    ;;   - slot initialization from initargs
    ;;   - struct.new $instance
    (list '(:ref.null :none))))
