  ...
  (cond
    ...
    ((keywordp instr)
     (case instr
       ...
       ;; f64 arithmetic (001-numeric-format)
       (:f64.abs (vector-push-extend #x99 buffer))
       (:f64.neg (vector-push-extend #x9A buffer))
       (:f64.floor (vector-push-extend #x9B buffer))
       (:f64.ceil (vector-push-extend #x9C buffer))
       (:f64.trunc (vector-push-extend #x9D buffer))
       (:f64.nearest (vector-push-extend #x9E buffer))
       (:f64.sqrt (vector-push-extend #x9F buffer))
       (:f64.add (vector-push-extend #xA0 buffer))
       (:f64.sub (vector-push-extend #xA1 buffer))
       (:f64.mul (vector-push-extend #xA2 buffer))
       (:f64.div (vector-push-extend #xA3 buffer))
       ;; i32 truncation from f64 (001-numeric-format)
       (:i32.trunc_f64_s (vector-push-extend #xAA buffer))
       (:i32.trunc_f64_u (vector-push-extend #xAB buffer))
