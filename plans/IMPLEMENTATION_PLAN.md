...
**目標**: （1）ホスト（SBCL等）で安定してAOTコンパイルできる状態を作り、（2）可能な環境では動的コンパイル（JIT）も提供する

**成果物**
- ホストCLで動くクロスコンパイラ（Wasmエミッタ含む）
- `compiler.wasm`（Lispで書かれたコンパイラ本体。最初はホストでクロスコンパイルして生成）
- オプション: 実行時にコンパイル結果をロードして関数セルへ登録できる仕組み（JIT）

#### 3.1 ブートストラップ方針（JSではなくホストCLを使う）

```
Phase 0: ホストCLで Wasmエミッタ（WAT/wasm生成）を作る
Phase 1: ホストCLで「サブセットCL → Wasm」クロスコンパイラを作る（まずAOT）
Phase 2: 上記で compiler.wasm（コンパイラをLispで実装）を生成する
Phase 3: compiler.wasm が自身を再コンパイルできる（セルフホスティング検証）
```

ポイント:
- 「JSでコンパイラを書く」は最小グルー（instantiate/登録）に限定し、言語処理系の中核はホストCLで育てる
- まずは Node.js 等の制約が少ない環境でセルフホストを成立させ、ブラウザは段階的に対応する

#### 3.2 コンパイルパイプライン（まずANF/SSA、CPSは後回し）

```
S式 (list)
  ↓
マクロ展開（ホストCLのmacroexpandで可。将来はターゲット側マクロへ移行）
  ↓
AST化 + 正規化（ANF/SSA など。CPSは必要になってから）
  ↓
クロージャ変換（自由変数解析 → 環境引数化）
  ↓
Wasmコード生成（まずは「ランタイムAPI呼び出し中心」で正しさ優先）
  ↓
リンク/登録（AOTはビルド時、JITは実行時）
```

#### 3.3 ティアード実行モデル（M2インタプリタ + M3コンパイル）

- Tier 1（常に利用可能）: Wasm内インタプリタで `eval`
- Tier 2（高速化）: 関数をWasmコードへコンパイルして実行
  - **AOT（推奨/基本）**: ビルド時にまとめてWasm化し、配布物に含める（CSPの影響を受けない）
  - **JIT（オプション）**: 実行時にwasmバイナリを生成 → JSで `WebAssembly.instantiate()` → 関数セルへ登録

#### 3.4 ブラウザの制約（CSP/セキュリティ）とフォールバック

- ブラウザは「Wasm内で生成したデータをそのままコードとして実行」できない（ハーバード型）
- `WebAssembly.instantiate(bytes)` による動的モジュール生成は可能だが、CSPにより `wasm-unsafe-eval` 等が要求されて禁止される場合がある
- 方針:
  - JITは **環境依存の最適化** と割り切り、失敗時はTier 1へ確実にフォールバック
  - ブラウザ向けはAOTを主軸にし、JITを使う場合は要件（CSP設定/実行環境）を明記する

#### 3.5 最適化（段階導入）

- まず正しさ: インタプリタと同じ意味論（動的束縛・非局所脱出を含む）を保つ
- その後:
  - 末尾呼び出し最適化（`return_call` など。サポート状況を見つつ）
  - 既知プリミティブの直結/インライン化
  - Fixnumのアンボックス化（ホットパス限定）
  - 定数畳み込み、デッドコード除去
  - 代表ベンチ（fib/fact/リスト処理）で効果測定
